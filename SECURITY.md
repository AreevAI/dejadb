...
> **Status:** DejaDB is pre-1.0 (`0.1.x`). The security model below is honest
...
Being pre-1.0, only the latest published `0.1.x` release line receives security
fixes. Once `1.0` ships, this table will be updated with a support window.
...
- **Encryption-at-rest uses the underlying storage engine's AES-256-GCM**, which
  is a pre-1.0 experimental feature of that dependency. Treat it as
