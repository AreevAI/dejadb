...
with [maturin](https://github.com/PyO3/maturin). Encryption at rest is currently
a CLI feature, not exposed through the Python constructor.
...
a pre-1.0 experimental feature — treat it as defense-in-depth, not a substitute
...
around **28µs**, the `entity_latest` point read around **7µs**, and a
50ms-cadence voice loop with live write-back recalls at about **72µs p50 /
153µs p99** per frame. Benchmarks and methodology live in `crates/dejadb-bench`.
...
**Not yet — it is pre-1.0 (`0.1.x`).** The engine is built and tested (a large
test suite runs locally), but the API may still change and some features are
experimental. Read [`SECURITY.md`](SECURITY.md) for an honest account of what is
and isn't protected today, and treat it as beta software: safe for local and
trusted-environment use, not yet hardened for hostile multi-tenant deployment.

### Is DejaDB stable — can the format change under me?

The **on-disk `.mg` format and canonical serialization are frozen** (changing
them would break every content address and OMS conformance). The *Rust/CLI/Python
APIs* may still change before 1.0. Because the format is an open spec, your data
stays portable regardless.
