//! Deja Loop golden E2E tests — drive the governed self-improvement loop through
//! the real `deja` binary against a committed, deterministic dataset
//! (`golden/loop_generator.rs`), with the engine clock pinned via
//! `DEJA_LOOP_NOW_MS`, and assert the *exact* output of every surface: which
//! analyzers fire on which targets, the queue listings byte-for-byte
//! (recommendation content addresses included — the substrate stamps deja-loop
...
//! then `GOLDEN_BLESS=1 cargo test -p dejadb --test golden_loop_tests`
...
//! `deja-loop/` goldens means analyzer semantics, engine stamping, or a CLI
//! surface changed.

mod golden;

use golden::loop_generator::{generate_loop, DEJA_LOOP_NOW0_MS};
use golden::{
    assert_golden, deja, deja_at, import_loop_golden, loop_bundle_path, loop_golden_dir,
...
const T0: i64 = DEJA_LOOP_NOW0_MS;
const HOUR: i64 = 3_600_000;
const DAY: i64 = 86_400_000;

/// Run `deja loop <args..>` against `db` at pinned `now` — ns `agent`,
...
/// `deja-loop` that must exit 0; returns stdout.
...
/// `deja-loop list --format json` rows (default pending; pass e.g.
/// `&["--status", "all"]`).
...
fn recall_hook(db: &str, prompt: &str, extra: &[&str]) -> String {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_deja"))
        .args(&args)
        .env("DEJA_LOOP_NOW_MS", T0.to_string())
...
fn loop_now_seam_rejects_garbage() {
    // The simulation seam fails loud: a set-but-unparseable DEJA_LOOP_NOW_MS
    ...
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_deja"))
        ...
        .env("DEJA_LOOP_NOW_MS", "not-a-timestamp")
        .env_remove("DEJA_LOOP_POLICY")
        .output()
        .expect("spawn deja");
    assert!(!out.status.success(), "garbled DEJA_LOOP_NOW_MS must not succeed");
    assert!(
        String::from_utf8_lossy(&out.stderr).contains("DEJA_LOOP_NOW_MS"),
...
fn loop_recall_hook_injection_pinned() {
    ...
    let block_at = out.find("Deja Loop:").unwrap_or_else(|| panic!("no loop block: {out}"));
    ...
    assert!(!without.contains("pending recommendation"), "flagless hook leaked deja-loop: {without}");
}
...
fn loop_cli_mcp_parity() {
    ...
    // MCP leg on a fresh import: the dejadb_loop tool runs the engine and
    ...
    let resp = String::from_utf8_lossy(&out.stdout)
        ...
        .find(|v| v["id"] == 2)
        .expect("deja-loop response");
    assert_ne!(resp["result"]["isError"], true, "mcp deja-loop errored: {resp}");
