//! Deja Loop golden dataset generator — a deterministic memory file in which
...
//! golden tests run the real `deja` binary with `DEJA_LOOP_NOW_MS` pinned to the
...
/// The pinned "now" the golden suite runs the engine at (`DEJA_LOOP_NOW_MS`).
/// Same instant as the dataset's base epoch: seeds are backdated from it.
pub const DEJA_LOOP_NOW0_MS: i64 = BASE_EPOCH_MS;
...
pub fn generate_loop(work_dir: &Path, bundle_path: &Path) -> Manifest {
    ...
    let mut m = DejaDB::open(db_path.to_str().unwrap()).expect("open loop source store");
    ...
    for (i, (o, ts)) in [
        ("Enterprise", DEJA_LOOP_NOW0_MS - 40 * DAY_MS),
        ("enterprise", DEJA_LOOP_NOW0_MS - 30 * DAY_MS),
        ("Enterprise", DEJA_LOOP_NOW0_MS - 20 * DAY_MS),
    ]
    .iter()
    .enumerate()
    {
    ...
    for (i, (body, ts)) in [
        (obs_body.to_string(), DEJA_LOOP_NOW0_MS - 15 * DAY_MS),
        (format!("{obs_body} please"), DEJA_LOOP_NOW0_MS - 15 * DAY_MS + MIN_MS),
    ]
    .iter()
    .enumerate()
    {
    ...
    let h = fact(&mut m, "sam", "lives_in", "berlin", DEJA_LOOP_NOW0_MS - 10 * DAY_MS);
    push(&mut grains, h.to_hex(), "fact", "contradiction older: sam lives_in berlin");
    let h = fact(&mut m, "sam", "lives_in", "tokyo", DEJA_LOOP_NOW0_MS - 5 * DAY_MS);
    ...
    for (s, o, expired_days, desc) in [
        ...
        let mut f = Fact::new(s, "discount_code", o)
            .confidence(1.0)
            .valid_to(DEJA_LOOP_NOW0_MS - expired_days * DAY_MS);
        f.common.namespace = Some("agent".to_string());
        f.common.created_at = Some(DEJA_LOOP_NOW0_MS - 120 * DAY_MS);
    ...
    for i in 0..6u32 {
        ...
        t.common.created_at = Some(DEJA_LOOP_NOW0_MS - 2 * DAY_MS + (i as i64) * 5 * MIN_MS);
    ...
    s.common.created_at = Some(DEJA_LOOP_NOW0_MS - 20 * DAY_MS);
    ...
    g.common.created_at = Some(DEJA_LOOP_NOW0_MS - 45 * DAY_MS);
    ...
    let h = fact(&mut m, "kai", "prefers", "dark-mode", DEJA_LOOP_NOW0_MS - 3 * DAY_MS);
    ...
    let base = fact(&mut m, "deploy", "region", "us-east-1", DEJA_LOOP_NOW0_MS - 30 * DAY_MS);
    ...
    ap.common.created_at = Some(DEJA_LOOP_NOW0_MS - 8 * DAY_MS);
    ...
    eu.common.created_at = Some(DEJA_LOOP_NOW0_MS - 9 * DAY_MS);
