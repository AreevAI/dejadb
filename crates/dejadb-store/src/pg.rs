...
impl PgDb {
    ...
    pub(crate) fn open(url: &str, schema: &str, bootstrap: &[&str]) -> Result<Self> {
        ...
        rt.block_on(async {
            client
                .query_one(
                    "SELECT pg_advisory_lock(hashtext('dejadb_bootstrap'), hashtext($1))",
                    &[&schema],
                )
                .await
                .map_err(pg_err)?;
            let boot = async {
                ...
                for sql in bootstrap {
                    client.batch_execute(sql).await.map_err(|e| {
                        DejaDbError::Storage(format!("bootstrap failed: {} — in: {sql}", pg_err(e)))
                    })?;
                }
                Ok::<_, DejaDbError>(())
            }
            ...
            let _ = client
                .query_one(
                    "SELECT pg_advisory_unlock(hashtext('dejadb_bootstrap'), hashtext($1))",
                    &[&schema],
                )
                .await;
            boot
