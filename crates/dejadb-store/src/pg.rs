...
//! `pg_dump -n`, and erasure `DROP SCHEMA … CASCADE`). Single-writer-per-
//! memory is ENFORCED here, not advisory: open takes a session advisory lock
//! on the schema name and a second writer gets `STO-E002` instead of the
...
pub(crate) const PG_SCHEMA: &[&str] = &[
    ...
    "CREATE TABLE IF NOT EXISTS terms(id bigint PRIMARY KEY, term text UNIQUE)",
