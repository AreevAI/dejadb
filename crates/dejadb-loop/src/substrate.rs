//! `DejaDbSubstrate` — implements `deja_loop::OmsSubstrate` over `DejaDbFacade`.
//!
...
//!
//! 1. **Deja Loop-internal grains ride as Facts.** The recommendation grain
//!    (OMS 0x0C) is not yet realized in dejadb-core, so recommendation and
//!    audit grains are stored as Facts in the `deja-loop` namespace with the full
...
use deja_loop::error::{Error as WErr, Result as WResult};
use deja_loop::{
...
/// The namespace the loop's own grains live in (mirrors `deja_loop::LOOP_NS`).
const LOOP_NS: &str = "deja-loop";
...
impl DejaDbSubstrate {
    ...
    /// (defaults to the `deja-loop` namespace when `None`).
...
/// one per deja-loop call over the host object's existing `&DejaDbFacade`.
...
fn grain(f: &DejaDbFacade, hash: &str) -> WResult<Option<GrainRecord>> {
    ...
    let created = grain_created_ms(&g);
    // Reconstruct loop-internal grains from their JSON payload.
    if let Some(rel) = g.get_str("relation") {
        if let Some(out_type) = loop_relation_type(rel) {
            ...
            {
                return Ok(Some(loop_record(hash, out_type, created, fields)));
            }
        }
    }
...
/// engine's injected `now` instead of leaking the wall clock, so a deja-loop
...
fn execute_cal(f: &DejaDbFacade, cal: &str) -> WResult<Vec<Value>> {
    // Deja Loop proposals are a compact store-op form (ADD/SUPERSEDE/FORGET
