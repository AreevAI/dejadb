# Deja Loop Reflection Engine — governed, verified, measured self-improvement
...
approval-rate metric (`deja loop`). The **out-of-box multi-provider layer (§9)
is now built too**: the `dejadb-llm` crate (OpenAI-compatible + Anthropic +
Ollama adapters over `ureq`) + `deja loop run --model provider:name`, key from
...
Deja Loop already does the hard, verifier-correct half: **deterministic analyzers
...
because it maps one-to-one onto Deja Loop's existing split:
...
(memory/skills), not weight updates — the same class as Deja Loop.
...
ungoverned, unmeasured version of what Deja Loop does.** (Its headline popularity
...
That is already Deja Loop's shape — this proposal adds (c) and the measurement.
...
## 7. How it maps onto DejaDB / Deja Loop primitives
...
New surface required (small): a verifier module in `deja-loop` (objective, decompose,
...
6. **Approval-rate metric (§6b)** — a reader over the audit chain, surfaced in
   `deja loop` + the console Sessions/Setup views.
