# Deja Loop — self-improving agents with governance and guardrails, on DejaDB
...
engine was renamed Deja Loop in v1.1.1.)*
**Product**: Deja Loop — the self-improvement layer of the DejaDB
agent backend.
**Tagline**: *"Deja Loop makes your agent wiser — self-improvement with
receipts."*
...
**Deja Loop's bet: self-improvement is a governance problem before it is an
...
What we ship: **DejaDB + Deja Loop, the governed backend for self-improving
...
User surfaces use exactly two words, everywhere. **Deja Loop** names the
engine and every surface it owns: the `deja loop` verb family, the
Deja Loop console tab, `/api/loop/*`, `--with-loop`,
`loop_config`/`loop_state`, `loop-policy.json`, the `deja-loop` crate.
...
| Hybrid recall; pluggable EmbedBackend / CommandEmbed, RerankBackend, QueryExpander | Deja Loop engine crate: OmsSubstrate trait, reference substrate, Analyzer SDK, six analyzers, validate/dedup/store pipeline |
| Console `deja ui` (Memories / Graph / Query tabs, one embedded HTML file); HTTP API (`/api/cal`, stats, log, config, browse, grain, verify, segments) | Console pillars: Overview, Sessions, Deja Loop (Queue/Analyzers/Outcomes), Setup; `/api/loop/*`; **breaking auth change: token-less UI becomes read-only** |
| MCP server (6 tools); Python + Node bindings; ~24 CLI verbs incl. `migrate` (8 sources), `provenance`, `forks`/`merge`, `restore` | the `deja loop` verb family, `deja init`; MCP +2 tools; py/js `loop_run` + recommendation methods + `record_tool_call`; generic tool-log importer |
| Claude Code hooks: recall-hook (context in), capture-stop (tool calls + errors out as Event grains) | Third hook (SessionEnd → `deja loop run`); recall-hook `--with-loop` |
...
#     SessionEnd       → /abs/deja loop run --min-new 20 --min-new-errors 3 --quiet  (new)

# … work normally for a few sessions …

deja ui --db agent.db --token-env DEJA_TOKEN
#   Overview shows backend health; the Deja Loop tab shows the pending queue.
```
...
**The 60-second proof (docs and the README Deja Loop section open with this).**
...
`deja mcp` gains two tools: `dejadb_loop` and `dejadb_recommendations`.
Docs ship a system-prompt line — *"At session start call dejadb_loop;
...
```bash
deja loop run --db agent.db --min-new 20 --min-new-errors 3 --if-stale 6h
...
deja loop list --db agent.db --status pending --fail-on high --format json
#   CI gate: exit 0 = none match, 2 = pending high-severity exists
#   (build-blocking), 1 = error. JSON envelope is append-only like /api.
```

Time triggers are the OS's job, not ours — `deja loop schedule --print`
emits a correct, absolute-path snippet for the host's scheduler (there is
no `watch` verb, §15):

```bash
deja loop schedule --print --every 6h --os cron|launchd|systemd|windows
...
```bash
deja migrate --from mem0 export.json --db agent.db --loop   # analyze after import
```
...
deja loop run --db demo.db              # ~5 recommendations across analyzers
...
who brings logs, not a memory product). The first `deja loop run` on any
...
- **Fast on real files**: panels paged; target <100ms server time per panel
  ...
  T0 built-ins only, and point at `deja loop run` for the rest.
- **Keyboard-first where it matters**: queue navigation j/k/Enter; deep
  links for every entity (`?tab=deja-loop&rec=<hash>`,
...
after the engine repo split (§10): `dejadb` depends on the `deja-loop` crate
...
console build carries the Deja Loop tab. A backend that has never run the loop
...
The deja-loop repo ships no UI; its CI runs against the reference substrate
...
Tabs: **Overview | Memories | Graph | Query | Sessions | Deja Loop | Setup**.
Pending-recommendation count chips on the Deja Loop tab.
...
| Deja Loop | manage | sub-nav Queue \| Analyzers \| Outcomes + the detail view | new |
| Setup | create | live readiness checklist + editors + wiring snippets | new |

### 5.4 Manage pillar (the Deja Loop tab)
...
- **API** (new routes under `/api/loop/*`): GET recommendations; GET
  ...
  `GET /api/config` extended with deja-loop/policy reporting. `POST run`
  ...
  budget|tier|capability}], hint: "deja loop run"}` — the console renders
...
- **Setup tab** — a live checklist against the open file; each row = status
  ...
  Deja Loop-tab empty state shows the watermark math ("needs ≥20 new grains
...
- **Token-less `deja ui` = read-only.** CAL writes rejected server-side
  ...
  containing any write is a write); all Deja Loop mutations and Setup writes
  return 401 with a "restart with `--token-env`" hint. Overview, Memories,
  Graph, Query (reads), Sessions, Deja Loop (reads), and the Setup checklist
...
- Size estimate: Deja Loop tab ~+850 lines; Overview ~150; Sessions ~200;
...
- Creating-actor semantics: a deterministic analyzer run's recommendations
  ...
  `deja loop run`. The block compares the approving actor to the creating
...
- **One optional host-side JSON policy file**: `deja … --policy
  loop-policy.json` (env `DEJA_LOOP_POLICY`). serde_json only; no new deps;
...
is a standing DejaDB invariant, and Deja Loop inherits it.
...
a post-1.0 minor): `deja loop audit --export jsonl` for compliance
...
- **Apply carries its reason.** `apply_recommendation(hash, because=…)` is
  ...
  (the library-friendly name for `deja loop reject`).
...
Deja Loop writes a new grain type (0x0C) and new file tables into files that
...
- **Old reader, new grain.** OMS §4.5's "existing content addresses remain
  ...
  deja-loop release, a test must confirm that shipped DejaDB 1.0.x **skips**
...
**Runtime posture**: `deja loop run` is a batch verb outside every latency
...
Health Report (the CLI twin of Overview, printed by bare `deja loop`)
...
- **`deja-loop` = a standalone engine crate with zero dejadb dependencies.**
...
- **Repo timing**: develop as a workspace member in the dejadb repo with
  CI-enforced zero sibling deps; lift to `AreevAI/deja-loop` when semantics
  ...
  subtree move. After the split, `dejadb` consumes `deja-loop` from crates.io
...
- **Strategy note**: our earlier OMS play was spec-first — publish the
  ...
  architecture retries it product-first: Deja Loop becomes the reason to be
...
A new crate at the context tier (`core ← store ← cal ← deja-loop`; consumed
by cli/mcp/server/py/js through the substrate adapter).

```rust
pub trait Analyzer: Send + Sync {
    fn manifest(&self) -> &AnalyzerManifest;
    fn analyze(&self, ctx: &mut AnalyzeCtx) -> Result<Vec<RecDraft>, deja_loop::Error>;
}
```
...
3. **Linked Rust (the in-house seam)** — embedding hosts call
   `deja_loop::Engine::register(Box::new(...))`; proprietary crates stay
...
Deja Loop can learn from it."** Never claim "works with any framework" —
claim the mechanism.
...
audited `rejected` transition as `deja loop reject` — a library-friendly
...
**host applies in its own world** → host reports back via `deja loop
mark-applied <hash> --actor … --because …` (audited like every other
transition) → outcome review measures as usual. Deja Loop recommends beyond
...
- `examples/import/` — a ~30-line sample OpenAI-style tool-call JSONL + the
  walkthrough (`deja import --format tool-log … && deja loop run` →
...
- `examples/ci/` — a GitHub Actions job: `deja loop run` on the synced
  file → `deja loop list --status pending --fail-on high` → exit 2 fails
...
- CLI — one namespace, `deja loop`: bare `deja loop` prints the Memory
...
- **Run-outcome contract** (the reporting half of "hosts call a cheap
  ...
  - **Exit taxonomy**: `run` exits 0 on *ran or clean skip* (cron must not
    page on a healthy no-op) and 1 on error; the CI gate `deja loop list
  ...
  - **Human notification without a daemon or new surface**: a SessionEnd
    ...
    stderr line when new recommendations land (`deja-loop: 3 new (1 high) —
    deja loop list`); a non-quiet run prints the health report.
...
- **Trigger architecture: no scheduler, no daemon, anywhere.**
  `deja loop run --min-new 20 --min-new-errors 3 --if-stale 6h` no-ops
...
- **Concurrency & multi-host**: locally, `deja loop run` is an ordinary
...
- Implement 0x0C in DejaDB/deja-loop NOW as the reference implementation
...
| No `deja loop watch` verb — time triggers are the OS scheduler's job; we print its config (`schedule --print`) | A foreground `watch` respects the letter of no-daemon but not the spirit — users `nohup` it and then want pidfiles, logs, restart-on-crash, i.e. scheduler creep with worse reliability than the OS scheduler already on every platform. Reopen on sustained Windows Task Scheduler friction. |
| `run` exits 0 on clean skip; a held lock is a clean skip, not an error | Cron must not page on a healthy no-op; a raw busy error on stacked triggers reads as "Deja Loop is flaky." Exit 1 is reserved for real errors; the CI gate `list --fail-on` uses exit 2. |
...
0. **Prerequisite fixes** (~1 day): wire QueryRegistry hooks into
   ...
   extend `GET /api/config` with deja-loop/policy reporting.
1. **Engine crate** (`deja-loop`, workspace member, CI-enforced zero sibling
   ...
   a `LOP` error-code domain (the engine has zero dejadb deps, so it needs
...
2. **DejaDB adapter + CLI + bindings**: substrate impl over facade/store,
   the `deja loop` verb family (incl. `--format json`,
...
5. **Console increment 1 — Manage + Overview + auth** (~1 week): Paper
   ...
   change; Deja Loop tab (Queue/Detail/Analyzers/Outcomes); Overview +
...
8. **Docs** (~2–3 days): `docs/loop.md` (concepts, four gates, policy
   ...
   into `mcp-reference.md`; a README Deja Loop section (leading with the
...
9. **Examples + cookbook reconciliation** (~2–3 days): the top-level
   ...
   the exact loop Deja Loop governs (reflect-via-your-own-LLM, manual lessons,
   ...
   stories; a new canonical "Self-improve with Deja Loop" page carries the
...
telemetry-fed analyzers (2–4 weeks of telemetry exist) · `deja-loop replay`
(post-1.0 flagship, §17) · `--notify-cmd` (demand — the one-line stderr
notice covers launch) · interactive `deja loop review` walker (CLI-first
users ask) · per-namespace run gates (`--min-new-ns`, asked) ·
rollback-discovery sugar (`deja-loop list --status applied --since 7d`, plus
...
1. **Counterfactual replay — the flagship** (post-1.0): immutable history
   ...
   past offline (`deja-loop replay --set duplicate_sweep.threshold=0.85
...
Deja Loop's honest sentence: *it measures which of its own advice works and
...
`deja-loop` in v1.1.1, so these reservations are now moot — the
`deja-loop` / `dejadb-loop` crate names are the live ones.)*

**Resolved (2026-07-17) — branding and naming**: Deja Loop is adopted
outright as the user-facing name, under the two-word rule of §2.6: the
`deja loop` verb family, the Deja Loop tab inside the unchanged `deja ui`,
`/api/loop/*`, `--with-loop`. There is no separate Deja Loop console,
...
5. **OMS 1.5 timing** relative to the deja-loop repo split (currently
...
- **Error codes**: stable `DOMAIN-Ennn`, append-only, registry in
  ERROR_CODES.md, format/uniqueness test-enforced. Deja Loop adds codes in a
