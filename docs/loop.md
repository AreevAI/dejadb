# Deja Loop — governed self-improvement for AI agents

Deja Loop turns an agent's own history into **recommendations** — evidence-cited,
...
deja ui --db demo.db --token-env DEJA_TOKEN   # the Deja Loop tab shows the queue
```
...
- **GROUND → VERIFY** — before a draft is ever queued it must pass an
  ...
  **Effective Reliability**, and `deja loop` reports the live approval-rate of
...
Determinism you can extend without recompiling: `deja loop run --analyzer-cmd
...
rollback|config` (writes). The console's Deja Loop tab renders the queue with
...
outcomes — facts about data Deja Loop owns (did this tool fail again, does this
...
purpose. Deja Loop improves the agent's *memory*, not its *outputs* (§2.4).
...
The SessionEnd Claude Code hook runs `deja loop run --min-new 20
...
- **Interim grain mapping.** The OMS 1.5 `0x0C` Recommendation type **is** now
  realized in dejadb-core, but Deja Loop has not migrated to it: recommendation
  and audit grains still ride as Facts in the `deja-loop` namespace with the
