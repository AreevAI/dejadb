# Deja Loop — the explainer (slide source)

> Purpose: the narrative source for a slide deck explaining Deja Loop. **One H2 =
...
**Deja Loop** — governed self-improvement for AI agents, built into DejaDB.
...
*Visual: the word "Deja Loop" over the loop diagram from slide 5, greyed.*

*Speaker notes: Deja Loop ships inside DejaDB — the `deja loop` CLI family,
...
which is a review gate, i.e., the thing Deja Loop makes native. Reflection loops
...
DeepSeek-R1's rule-based rewards) compound. Deja Loop is built on that asymmetry.*

---

## 5. What Deja Loop is — the loop
...
- Runs on DejaDB's typed grains (facts, events, skills, tool calls) — never
  raw prose.
- Scope, stated plainly: Deja Loop improves the agent's **memory** — what it
...
- The utility column is the differentiator: a disposable, never-syncing
  telemetry sidecar records what recall actually surfaced — so Deja Loop sees
...
- Analyzer precision: **1.00 precision / 1.00 recall** on the labeled fixture
  ...
  **0.90**. (`cargo run -p dejadb-bench --bin loop_precision`)
...
- Live: `deja loop` reports the **approval rate** of LLM findings from the
...
- **The honest boundary**: this works for internal, bounded, attributable
  ...
  *not* score open-ended world outcomes (was the post good) — those stay a
  monitored trend a human judges. Deja Loop improves the agent's *memory*, not
  its *outputs*.
...
- **CI gate**: `deja loop list --fail-on high` exits 2 when a pending
  high-severity recommendation exists — build-blocking memory review.
- **Surfaces**: `deja loop` CLI · Python/Node (`loop_run`,
  ...
  proposals) · `/api/loop/*` HTTP · the console's Deja Loop tab (queue,
...
- Alternative from the CLI: `deja init --db demo.db --template demo` →
  `deja loop run` → `deja ui` (the Deja Loop tab shows the governed queue).
...
*Visual: the matrix itself, Deja Loop's row highlighted; or a 2×2 (x: proposes
automatically, y: governed+verified+measured) with Deja Loop alone top-right.*
...
## 15. What Deja Loop does NOT do (say it before they ask)
...
- Context injection is opt-in, not ambient: `recall-hook --with-loop` rides
  ...
  host **pulls** (an MCP call, a binding query, the console). Deja Loop never
...
(`deja loop reflect`) · `recall-hook --with-loop` context injection ·
...
1. **Pre-apply replay validation** — answer "will this change make my agent
   ...
   change; today Deja Loop verifies claims before and measures after.
2. **Substrate adapters beyond DejaDB** — the engine is substrate-agnostic
   ...
   mem0/Letta/LangGraph users run Deja Loop without migrating first
...
- `pip install dejadb` · `deja init` · `deja loop run` — the loop is
