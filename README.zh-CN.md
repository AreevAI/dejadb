...
进程内嵌入，把记忆存为不可变的内容寻址 (content-addressed) grain，用 CAL 查询，
结果直接交给模型 —— 召回路径上没有服务器、没有 sidecar、没有网络跳转。
**微秒级召回。你的智能体记忆，就是一个你自己拥有的文件。**

> 给智能体记忆的 git：日志 (log)、差异 (diff)、时间回溯、带显式合并的分叉，
> 以及加密增量同步 —— 全部内建于数据模型之中，因为 grain *本身就是*
> 内容寻址的不可变对象。

*状态：预发布（`0.1.0`）。已在本地构建并测试；尚未发布到
crates.io / PyPI / npm。*
...
> **▶ 演示视频 —— 即将推出。** 在此之前，
> [快速开始](#quickstart-cli) 就是一次 30 秒的上手之旅，而
> [`docs/cookbook.md`](docs/cookbook.md) 提供了面向任务的实用配方。
...
DejaDB 尚处于预发布阶段，因此目前请从源码安装（需 Rust 1.82+）：
