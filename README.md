...
[![MSRV](https://img.shields.io/badge/rustc-1.82%2B-blue.svg)](#install)

Embed it in-process, store memories as immutable content-addressed grains, query
them with CAL, and hand the results straight to a model — no server, no sidecars,
no network hop in the recall path. **Recall in microseconds. Your agent's memory
is a file you own.**
...
*Status: pre-release (`0.1.0`). Built and tested locally; not yet published to
crates.io / PyPI / npm.*

## Demo

<!-- ▶ Demo video / asciinema cast goes here. A shot-by-shot script for it lives
     in docs/demo-script.md. Until it's recorded, the quickstart below is the
     30-second tour. -->

> **▶ Walkthrough video — coming soon.** In the meantime, the
> [Quickstart](#quickstart-cli) is the 30-second tour, and
> [`docs/cookbook.md`](docs/cookbook.md) has task-oriented recipes.
...
- **CAL-native**: `RECALL` / `ASSEMBLE` / `EXISTS` / `HISTORY` / `ADD` /
  ...
  per process.
- **Fast where it matters** (measured, Apple M4 Max): structural recall **~28µs**,
  `entity_latest` **~7µs**, 50ms-cadence voice loop with live write-back
  **72µs p50 / 153µs p99** per frame recall.
...
DejaDB is pre-release, so install from source for now (Rust 1.82+):
...
reserved and land with the `0.1.0` release.
...
Reproducible harnesses in `crates/dejadb-bench` — full methodology and raw data
in its [`RESULTS.md`](crates/dejadb-bench/RESULTS.md); committed transcripts in
...
|---|---|---|
| `entity_latest` (in-process) | **~7 µs** | — |
| structural recall (in-process) | **~28 µs** | — |
| inside a 50 ms voice frame, live write-back | **72 µs** | 153 µs |
