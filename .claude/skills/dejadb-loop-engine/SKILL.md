---
name: dejadb-loop-engine
description: Playbook for the Deja Loop self-improvement engine (crates/deja-loop + dejadb-loop adapter) — adding or changing an analyzer, the four gates, the recommendation lifecycle/state machine, the DISCOVER→GROUND→VERIFY LLM verifier, and outcome measurement. Use before touching crates/deja-loop/src/{analyzer,engine,recommendation,policy,llm}.rs, anything under crates/deja-loop/src/analyzers/, or the dejadb-loop substrate adapter — the engine is substrate-agnostic and gate-governed, so a lone analyzer that skips the metric/gate wiring silently half-works.
---

# The Deja Loop engine

Deja Loop is a **substrate-agnostic**, **deterministic-core** self-improvement
engine: it reads typed grains (never raw prose), proposes governed
recommendations, and measures their outcomes. The `deja-loop` crate has **no
DejaDB dependency** — it speaks the `OmsSubstrate`/`SubstrateRead` traits
(`substrate.rs`) and `LlmBackend` (`llm.rs`); `dejadb-loop` is the adapter
...
`deja-loop/`, it belongs in `dejadb-loop` instead.
...
Eleven built-ins live in `crates/deja-loop/src/analyzers/` (`tool_failure`,
...
```bash
cargo test -p deja-loop              # core engine, analyzers, lifecycle, verifier
cargo test -p dejadb-loop       # the substrate adapter over DejaDbFacade
cargo test -p dejadb --test golden_loop_tests   # exact CLI-surface output
```

Regenerate the loop golden dataset only on an intended change:
`cargo test -p dejadb --test golden_loop_tests -- --ignored bless` then
`GOLDEN_BLESS=1 cargo test -p dejadb --test golden_loop_tests` (the clock is
pinned by `DEJA_LOOP_NOW_MS`; see [[dejadb-testing]]). Then run the
[[dejadb-invariants]] gate. New user-facing surface (CLI verb, `/api/loop/*`,
