...
`deja loop run --analyzer-cmd 'CMD'` registers a subprocess analyzer written
...
deja loop run  --db demo.db --analyzer-cmd 'python3 examples/analyzers/pii_scan.py'
deja loop list --db demo.db          # the [external] finding sits in the queue
```
...
`Analyzer` trait). Full contract: the module doc of `crates/deja-loop/src/external.rs`
